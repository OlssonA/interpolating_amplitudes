...
class SG:
    def __init__(self, d, n, function, point_threshold, bspline_degree, 
                step_size, saved_grid, verbose = True,
        ...
        self.set_extension_coefficients()

        ######### Setup the grid, look for saved results  ########
        if saved_grid:
            self.refinement_candidates = \
                deepcopy(saved_grid.refinement_candidates)
    ...
    #Pre-computed extension coefficients (for p=1,3,5)
    def set_extension_coefficients(self):
        if self.p == 1:
            self.extension_coeffs = {l : {} for l in range(1, self.n+1)}
            for l in range(1, self.n+1):
                self.extension_coeffs[l][0] = {1 : 2, 
                                               2 : -1}
                self.extension_coeffs[l][2**l] = {2**l-2 : -1, 
                                                  2**l-1 : 2}
        elif self.p == 3:
            self.extension_coeffs = {l : {} for l in range(2, self.n+1)}
            for l in range(2, self.n+1):
                self.extension_coeffs[l][0] = {1 : 5, 
                                               2 : -10,
                                               3 : 10,
                                               4 : -4}
                self.extension_coeffs[l][2**l] = {
                                                2**l - 4 : -4,
                                                2**l - 3 : 10,
                                                2**l - 2 : -10,
                                                2**l - 1 : 5}
        elif self.p == 5:
            self.extension_coeffs = {l : {} for l in range(3, self.n+1)}
            for l in range(3, self.n+1):
                if l == 3:
                    self.extension_coeffs[3][0] = {
                                                1 : 8,
                                                2 : -28,
                                                3 : 42,
                                                4 : -35,
    ...
    #Not-a-knot construction for the B-splines.
    def knot(self, k, l): 
        if l < self.Lambda_e:
            return k*2**-l
        else:
            if 0 <= k <= self.p: return (k-self.p)*2**(-l)
            elif self.p+1 <= k <= 2**l: return (k-(self.p+1)/2)*2**(-l)
    ...
    def bspline_basis(self, x, l, i, p):
        if p == 0:
            ...
            b = (self.knot(i+p+1, l) - x)/ \
                (self.knot(i+p+1, l) - self.knot(i+1,l))
            return a*self.bspline_basis(x, l, i, p-1)+\
                    b*self.bspline_basis(x, l, i+1, p-1)
    
    #Add outermost basis functions to p+1 interior basis functions
    def extended_nak_basis(self, x, l, i):
        if l < self.Lambda_e: return self.Lagrange_poly_basis(x, l, i)
        else:
            return self.bspline_basis(x, l, i, self.p)+\
                np.sum([self.extension_coeffs[l][j][i]*\
                        self.bspline_basis(x, l, j, self.p)
                        for j in self.extension_indices(l, i)])

    #Determine of outermost basis function should get added to basis function of 
    #index i. There can be overlap such that both left and right-most get added
    def extension_indices(self, l, i):
        extension_set = []
        if 1 <= i <= self.p+1: extension_set.append(0)
        if 2**l - 1 - self.p <= i <= 2**l - 1: extension_set.append(2**l)
        return extension_set
    
    #Loop over and evaluate all basis functions. This can be significantly
    #optimized by considering the support of the B-splines.
    def evaluate(self, x):
        result = 0
        for i in range(len(self.used_points)):
            basis_product = 1
            for d in range(self.dim):
                basis_product *= self.extended_nak_basis(
                                                    x[d], self.levels[i][d],
                                                    self.indices[i][d])
                if math.isclose(basis_product, 0, abs_tol = 1e-10): break
    ...
    def adaptive_spatial_refinement(self):
        ...
        def hierarchize():
            ...
            for point in self.used_points:
                ...
                for level, index in zip(self.levels, self.indices):
                    row.append(np.prod(
                        [self.extended_nak_basis(point[d], level[d], index[d]) 
                         for d in range(self.dim)]))
                matrix.append(row)
            if self.verbose: 
            ...
            while refinements < nr_refinements:
                ...
                for d in range(self.dim):
                    ...
                    lower_index, upper_index = \
                        copy(worst_index), copy(worst_index)
                    ...
                    for refined_index in [lower_index, upper_index]:
                        ...
                        refined_point[d] = \
                            refined_index[d]*2**(-refined_level[d])
                        if not refined_point in self.used_points:
                            add_point(refined_point, refined_level, 
                                      refined_index)
        ...
        if self.check_interpolated_points:
            tol = 1e-10
            errors = [abs(self.function(point) - self.evaluate(point)) 
                        for point in self.used_points]
            bad_indices = \
                [i for i in range(len(errors)) if errors[i] > tol]
