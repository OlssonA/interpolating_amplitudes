...
if __name__ == "__main__":
    ...
    q1 = np.column_stack(
        (np.ones(len(data)), np.zeros(len(data)), np.zeros(len(data)), np.sqrt(s) / 2)
    )
    q2 = np.column_stack(
        (np.ones(len(data)), np.zeros(len(data)), np.zeros(len(data)), -np.sqrt(s) / 2)
    )
    p1 = np.column_stack(
        (
            np.ones(len(data)),
            np.zeros(len(data)),
            -np.sin(theta_h) * factor,
            -np.cos(theta_h) * factor,
        )
    )
    p2 = np.column_stack(
        (
            (s + mh2) / (2 * np.sqrt(s)),
            np.zeros(len(data)),
            (s - mh2) * np.sin(theta_h) / (2 * np.sqrt(s)),
            (s - mh2) * np.cos(theta_h) / (2 * np.sqrt(s)),
        )
    )
