module     p0_ubaru_httbar_model_qp
   ...
   real(ki) :: sw =        0.480832611206852_ki
   ...
   real(ki),parameter :: wZ =        0.000000000000000_ki
   real(ki) :: NA
   real(ki) :: gZ
   real(ki) :: gW
   real(ki) :: gUv
   ...
   character(len=name_length), dimension(18) :: names = (/&
      & "alpha", &
      & "e    ", &
      & "GF   ", &
      & "gs   ", &
...
!---#[ print_parameter:
   ...
   write(unit,'(A1,1x,A11,G23.16,"const.")') "#", "mBMS = ", mBMS
   ...
   write(unit,'(A1,1x,A11,G23.16,"const.")') "#", "wtau = ", wtau
   write(unit,'(A1,1x,A11,G23.16,"const.")') "#", "wW   = ", wW
   write(unit,'(A1,1x,A11,G23.16,"const.")') "#", "wZ   = ", wZ
   if(is_verbose) then
   ...
   recursive subroutine set_parameter(name, re, im, ierr)
     ...
     &3383279502884197169399375105820974944592307816406286209_ki
      ...
      if (name.eq."aS" .or. name.eq."alphaS" .or. name.eq."alphas") then
         ...
         must_be_real = .true.
      elseif (name.eq."alphaEW" .or. name.eq."alpha") then
         alpha = re
         must_be_real = .true.
...
!---#[ subroutine init_functions:
   subroutine     init_functions()
     ...
     &3383279502884197169399375105820974944592307816406286209_ki
      ...
      gHtau=-1.0_ki/2.0_ki*mtau*mW**(-1)*sw**(-1)
      ...
      gXtau=1.0_ki/2.0_ki*mtau*mW**(-1)*sw**(-1)
...
!---#] EW scheme choice:
!---#] EW scheme choice:
end module p0_ubaru_httbar_model_qp
