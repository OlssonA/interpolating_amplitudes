module     p0_ubaru_httbar_dipoles_qp
   use p0_ubaru_httbar_config, only: ki => ki_qp
   use p0_ubaru_httbar_color_qp, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2, &
       & T1T4, &
       & T1T5, &
       ...
       & T2T5, &
       & T4T5
   use p0_ubaru_httbar_kinematics_qp, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_ff(mu_sq, vec, I, J)
      ...
      if (flag) then
              matrix(:,:) = T4T5(:,:)/CF
              term = V_SING(s_jk, real(mT, ki), real(mT, ki))
              term(1) = term(1) + log_s * term(2)
              term =  CF * term + GAMMA_F(real(mT, ki))
   ...
   function I_if(mu_sq, vec, I, J)
      ...
      if (flag) then
              matrix(:,:) = T1T4(:,:)/CF
              term = V_SING(s_ja, mT, 0.0_ki)
              term(1) = term(1) + log_s * term(2)
              term = CF * term + GAMMA_F(mT)
   ...
   function I_ff_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(mH, ki), real(mT, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(0.0_ki)*(-2.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(0.0_ki)*(-2.0_ki)/9.0_ki
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(mT, ki), real(mT, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(-2.0_ki)*(2.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(-2.0_ki)*(2.0_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
   ...
   function I_if_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, mT, 0.0_ki)
              term=term*ncs*(-1.0_ki)*(-2.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
...
end module p0_ubaru_httbar_dipoles_qp
