module     p2_gg_httbar_dipoles_qp
   use p2_gg_httbar_config, only: ki => ki_qp
   use p2_gg_httbar_color_qp, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2, &
       & T1T4, &
       & T1T5, &
       ...
       & T2T5, &
       & T4T5
   use p2_gg_httbar_kinematics_qp, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_if_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term=term*ncs*(0.0_ki)*(-2.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
...
end module p2_gg_httbar_dipoles_qp
