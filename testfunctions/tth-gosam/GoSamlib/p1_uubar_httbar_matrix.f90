...
contains
   ...
   pure function prefactor()
      ...
      use p0_ubaru_httbar_model, only: NC
      use p0_ubaru_httbar_kinematics, only: &
      & in_helicities, symmetry_factor
      use p0_ubaru_httbar_color, only: incolors
      implicit none
   ...
   subroutine     initgolem(is_first)
      use p0_ubaru_httbar_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p0_ubaru_httbar_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, fpprec, spprec, numprec, ok, h)
      use p0_ubaru_httbar_matrix, only: orig_sub => samplitude
      implicit none
      ...
      if (present(ok)) then
         if (present(h)) then
            call orig_sub(new_vecs, scale2, amp, prec, fpprec, spprec, numprec, ok, h)
         else
            call orig_sub(new_vecs, scale2, amp, prec, fpprec, spprec, numprec, ok)
         end if
      else
         call orig_sub(new_vecs, scale2, amp, prec, fpprec, spprec, numprec, ok)
      end if
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p0_ubaru_httbar_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p0_ubaru_httbar_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   subroutine OLP_color_correlated(vecs,ampcc)
      use p0_ubaru_httbar_matrix, only: orig_func => OLP_color_correlated
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   subroutine OLP_spin_correlated_lo2(vecs, ampsc)
      use p0_ubaru_httbar_matrix, only: orig_func => OLP_spin_correlated_lo2
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
...
end module p1_uubar_httbar_matrix
