module     olp_module
   ...
   public :: OLP_PrintParameter, OLP_EvalBornProcess
...
contains
   ...
   & bind(C,name="olp_start_")
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_matrix, only: p0_ubaru_httbar_initgolem => initgolem
      use p0_ubaru_httbar_config, only: p0_ubaru_httbar_PSP_rescue => PSP_rescue, &
           & p0_ubaru_httbar_PSP_verbosity => PSP_verbosity, &
           & p0_ubaru_httbar_PSP_chk_th1 => PSP_chk_th1, &
           & p0_ubaru_httbar_PSP_chk_th2 => PSP_chk_th2, &
           & p0_ubaru_httbar_PSP_chk_th3 => PSP_chk_th3, &
           & p0_ubaru_httbar_PSP_chk_kfactor => PSP_chk_kfactor, &
           & p0_ubaru_httbar_nlo_prefactors => nlo_prefactors
      use p2_gg_httbar_matrix, only: p2_gg_httbar_initgolem => initgolem
      use p2_gg_httbar_config, only: p2_gg_httbar_PSP_rescue => PSP_rescue, &
           & p2_gg_httbar_PSP_verbosity => PSP_verbosity, &
           & p2_gg_httbar_PSP_chk_th1 => PSP_chk_th1, &
           & p2_gg_httbar_PSP_chk_th2 => PSP_chk_th2, &
           & p2_gg_httbar_PSP_chk_th3 => PSP_chk_th3, &
           & p2_gg_httbar_PSP_chk_kfactor => PSP_chk_kfactor, &
           & p2_gg_httbar_nlo_prefactors => nlo_prefactors
      implicit none
      ...
      ! PSP_chk_th3 = 5
      ! PSP_chk_kfactor = -1
      ! p0_ubaru_httbar_PSP_rescue = PSP_rescue
      ! p0_ubaru_httbar_PSP_verbosity =  PSP_verbosity
      ! p0_ubaru_httbar_PSP_chk_th1 = PSP_chk_th1
      ! p0_ubaru_httbar_PSP_chk_th2 = PSP_chk_th2
      ! p0_ubaru_httbar_PSP_chk_th3 = PSP_chk_th3
      ! p0_ubaru_httbar_PSP_chk_kfactor = PSP_chk_kfactor
      ! p2_gg_httbar_PSP_rescue = PSP_rescue
      ! p2_gg_httbar_PSP_verbosity =  PSP_verbosity
      ! p2_gg_httbar_PSP_chk_th1 = PSP_chk_th1
      ! p2_gg_httbar_PSP_chk_th2 = PSP_chk_th2
      ! p2_gg_httbar_PSP_chk_th3 = PSP_chk_th3
      ! p2_gg_httbar_PSP_chk_kfactor = PSP_chk_kfactor
      if(stage.lt.0) then
         call p0_ubaru_httbar_initgolem(.true.)
         call p2_gg_httbar_initgolem(.false.)
      else
         call p0_ubaru_httbar_initgolem(.true.,stage,rndseed)
         call p2_gg_httbar_initgolem(.false.,stage,rndseed)
      end if
   ...
   use p0_ubaru_httbar_version, only: gosamversion, gosamrevision
   ...
   & bind(C,name="olp_setparameter_")
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_model, only: p0_ubaru_httbar_set_parameter => set_parameter
      use p2_gg_httbar_model, only: p2_gg_httbar_set_parameter => set_parameter
      implicit none
      ...
      call p0_ubaru_httbar_set_parameter(variable_name(1:l),real_part,imag_part,success)
      if(success==0) then ! return immediately on error
          return
      end if
      call p2_gg_httbar_set_parameter(variable_name(1:l),real_part,imag_part,success)
      if(success==0) then ! return immediately on error
          return
      end if
   end subroutine
   ...
   ! BLHA2 interface
   subroutine     OLP_EvalSubProcess2(label, momenta, mu,  parameters, res, acc) &
   ...
   subroutine     OLP_EvalBornProcess(label, momenta, parameters, res) &
      & bind(C,name="olp_evalbornprocess_")
         use, intrinsic :: iso_c_binding
         implicit none
         integer(kind=c_int), intent(in) :: label
         real(kind=c_double), dimension(50), intent(in) :: momenta
         ...
         select case(label)
         case(0)
                 call evalborn0(momenta(1:25), parameters, res)
         case(1)
                 call evalborn1(momenta(1:25), parameters, res)
         case(2)
                 call evalborn2(momenta(1:25), parameters, res)
         case default
            res = 0.0d0
         end select
   
      end subroutine OLP_EvalBornProcess

   subroutine     OLP_Finalize() &
   & bind(C,name="olp_finalize_")
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_matrix, only: p0_ubaru_httbar_exitgolem => exitgolem
      use p2_gg_httbar_matrix, only: p2_gg_httbar_exitgolem => exitgolem
      implicit none
      call p0_ubaru_httbar_exitgolem(.false.)
      call p2_gg_httbar_exitgolem(.true.)
   end subroutine OLP_Finalize

   subroutine     OLP_Option(line,stat) &
   & bind(C,name="olp_option_")
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_model, only: p0_ubaru_httbar_parseline => parseline
      use p0_ubaru_httbar_model_qp, only: p0_ubaru_httbar_parseline_qp => parseline
      use p2_gg_httbar_model, only: p2_gg_httbar_parseline => parseline
      use p2_gg_httbar_model_qp, only: p2_gg_httbar_parseline_qp => parseline
      implicit none
      ...
      l = strlen(line)
      call p0_ubaru_httbar_parseline(line(1:l),ios)
      call p0_ubaru_httbar_parseline_qp(line(1:l),ios)
      if (ios .ne. 0) then
         stat = 0
         return
      end if
      call p2_gg_httbar_parseline(line(1:l),ios)
      call p2_gg_httbar_parseline_qp(line(1:l),ios)
      if (ios .ne. 0) then
         stat = 0
         return
      end if
   ...
   subroutine     eval0(momenta, mu, parameters, res, acc, blha1_mode)
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_config, only: ki, PSP_chk_th3, nlo_prefactors, PSP_check
      use p0_ubaru_httbar_model, only: parseline
      use p0_ubaru_httbar_kinematics, only: boost_to_cms
      use p0_ubaru_httbar_matrix, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p0_ubaru_httbar_groups, only: ninja_exit, quadninja_exit
      ...
      real(kind=ki), dimension(5) :: amp
      real(kind=c_double), optional :: acc
      logical, optional :: blha1_mode
      real(kind=ki) :: zero
      integer :: i, prec, orig_nlo_prefactors, fpprec, spprec, numprec
      logical :: ok
      ...
      call samplitude(vecs, real(mu,ki)*real(mu,ki), amp, prec, fpprec, spprec, numprec, ok)
      call ninja_exit()
      call quadninja_exit()
      if (ok) then
         !
   ...
   subroutine     eval1(momenta, mu, parameters, res, acc, blha1_mode)
      ...
      use p0_ubaru_httbar_model, only: parseline
      use p0_ubaru_httbar_kinematics, only: boost_to_cms
      use p1_uubar_httbar_matrix, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p0_ubaru_httbar_groups, only: ninja_exit, quadninja_exit
   ...
   subroutine     eval2(momenta, mu, parameters, res, acc, blha1_mode)
      use, intrinsic :: iso_c_binding
      use p2_gg_httbar_config, only: ki, PSP_chk_th3, nlo_prefactors, PSP_check
      use p2_gg_httbar_model, only: parseline
      use p2_gg_httbar_kinematics, only: boost_to_cms
      use p2_gg_httbar_matrix, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p2_gg_httbar_groups, only: ninja_exit, quadninja_exit
   ...
   !---#[ subroutine evalborn0 :
   subroutine     evalborn0(momenta, parameters, res)
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_config, only: ki
      use p0_ubaru_httbar_model, only: parseline
      use p0_ubaru_httbar_kinematics, only: boost_to_cms
      use p0_ubaru_httbar_matrix, only: samplitudel0
      use p0_ubaru_httbar_groups, only: ninja_exit, quadninja_exit
      ...
      amp = samplitudel0(vecs)
      call ninja_exit()
      call quadninja_exit()

      
      res = real(amp(1), c_double)

   end subroutine evalborn0
   !---#] subroutine evalborn0 :
   !---#[ subroutine evalborn1 :
   subroutine     evalborn1(momenta, parameters, res)
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_config, only: ki
      use p0_ubaru_httbar_model, only: parseline
      use p0_ubaru_httbar_kinematics, only: boost_to_cms
      use p1_uubar_httbar_matrix, only: samplitudel0
      use p0_ubaru_httbar_groups, only: ninja_exit, quadninja_exit
   ...
   end subroutine evalborn1
   !---#] subroutine evalborn1 :
   !---#[ subroutine evalborn2 :
   subroutine     evalborn2(momenta, parameters, res)
      use, intrinsic :: iso_c_binding
      use p2_gg_httbar_config, only: ki
      use p2_gg_httbar_model, only: parseline
      use p2_gg_httbar_kinematics, only: boost_to_cms
      use p2_gg_httbar_matrix, only: samplitudel0
      use p2_gg_httbar_groups, only: ninja_exit, quadninja_exit
   ...
   subroutine OLP_Polvec(p,q,eps) &
      ...
      use p0_ubaru_httbar_kinematics, only: Spab3, Spaa   
      implicit none
   ...
   subroutine OLP_PrintParameter(filename) &
      ...
      use, intrinsic :: iso_c_binding
      use p0_ubaru_httbar_model, only: p0_ubaru_httbar_print_parameter => print_parameter
      use p2_gg_httbar_model, only: p2_gg_httbar_print_parameter => print_parameter
      implicit none
      ...
      write (27, "(A)") "####### Setup of SubProcess p0_ubaru_httbar #######"
      call p0_ubaru_httbar_print_parameter(.true.,27)
      write (27, *)
      write (27, "(A)") "####### Setup of SubProcess p2_gg_httbar #######"
      call p2_gg_httbar_print_parameter(.true.,27)
      write (27, *)
   ...
   subroutine     read_slha_file(line)
      use p0_ubaru_httbar_model, only: p0_ubaru_httbar_read_slha => read_slha
      use p0_ubaru_httbar_model_qp, only: p0_ubaru_httbar_read_slha_qp => read_slha
      use p2_gg_httbar_model, only: p2_gg_httbar_read_slha => read_slha
      use p2_gg_httbar_model_qp, only: p2_gg_httbar_read_slha_qp => read_slha
      implicit none
      ...
      if(ierr.ne.0) then
         ...
         call p0_ubaru_httbar_read_slha(27)
         call p0_ubaru_httbar_read_slha_qp(27)
         rewind(unit=27)
         call p2_gg_httbar_read_slha(27)
         call p2_gg_httbar_read_slha_qp(27)
         close(27)
      end if
