...
contains
   ...
   & bind(C,name="olp_start_")
      use, intrinsic :: iso_c_binding
      use p0_gg_gh_matrix, only: p0_gg_gh_initgolem => initgolem
      use p0_gg_gh_config, only: p0_gg_gh_PSP_rescue => PSP_rescue, &
           & p0_gg_gh_PSP_verbosity => PSP_verbosity, &
           & p0_gg_gh_PSP_chk_li1 => PSP_chk_li1, &
           & p0_gg_gh_PSP_chk_li2 => PSP_chk_li2, &
           & p0_gg_gh_PSP_chk_li3 => PSP_chk_li3, &
           & p0_gg_gh_PSP_chk_li4 => PSP_chk_li4, &
           & p0_gg_gh_PSP_chk_li5 => PSP_chk_li5, &
           & p0_gg_gh_PSP_chk_li6 => PSP_chk_li6, &
           & p0_gg_gh_PSP_chk_kfactor => PSP_chk_kfactor
      implicit none
      ...
      ! p0_gg_gh_PSP_rescue = PSP_rescue
      ! p0_gg_gh_PSP_verbosity =  PSP_verbosity
      ! p0_gg_gh_PSP_chk_li1 = PSP_chk_li1
      ! p0_gg_gh_PSP_chk_li2 = PSP_chk_li2
      ! p0_gg_gh_PSP_chk_li3 = PSP_chk_li3
      ...
      ! p0_gg_gh_PSP_chk_kfactor = PSP_chk_kfactor
      if(stage.lt.0) then
         call p0_gg_gh_initgolem(.true.)
      else
         call p0_gg_gh_initgolem(.true.,stage,rndseed)
      end if
   ...
   use p0_gg_gh_version, only: gosamversion, gosamrevision
   ...
   & bind(C,name="olp_setparameter_")
      use, intrinsic :: iso_c_binding
      use p0_gg_gh_model, only: p0_gg_gh_set_parameter => set_parameter
      implicit none
      ...
      call p0_gg_gh_set_parameter(variable_name(1:l),real_part,imag_part,success)
   ...
   & bind(C,name="olp_finalize_")
      use, intrinsic :: iso_c_binding
      use p0_gg_gh_matrix, only: p0_gg_gh_exitgolem => exitgolem
      implicit none
      call p0_gg_gh_exitgolem(.true.)
   end subroutine OLP_Finalize

   subroutine     OLP_Option(line,stat) &
   & bind(C,name="olp_option_")
      use, intrinsic :: iso_c_binding
      use p0_gg_gh_model, only: p0_gg_gh_parseline => parseline
      use p0_gg_gh_model_qp, only: p0_gg_gh_parseline_qp => parseline
      implicit none
      ...
      l = strlen(line)
      call p0_gg_gh_parseline(line(1:l),ios)
      call p0_gg_gh_parseline_qp(line(1:l),ios)
      if (ios .ne. 0) then
         stat = 0
         return
      end if
   ...
   subroutine     eval0(momenta, mu, parameters, res, acc, blha1_mode)
      use, intrinsic :: iso_c_binding
      use p0_gg_gh_config, only: ki, PSP_chk_li3, nlo_prefactors, PSP_check
      use p0_gg_gh_model, only: parseline
      use p0_gg_gh_kinematics, only: boost_to_cms
      use p0_gg_gh_matrix, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p0_gg_gh_groups, only: ninja_exit, quadninja_exit
   ...
   subroutine OLP_Polvec(p,q,eps) &
      ...
      use p0_gg_gh_model
      use p0_gg_gh_kinematics, only: Spab3, Spaa  
      implicit none
   ...
   subroutine OLP_PrintParameter(filename) &
      ...
      use, intrinsic :: iso_c_binding
      use p0_gg_gh_model, only: p0_gg_gh_print_parameter => print_parameter
      implicit none
      ...
      write (27, "(A)") "####### Setup of SubProcess p0_gg_gh #######"
      call p0_gg_gh_print_parameter(.true.,27)
      write (27, *)
   ...
   subroutine     read_slha_file(line)
      use p0_gg_gh_model, only: p0_gg_gh_read_slha => read_slha
      use p0_gg_gh_model_qp, only: p0_gg_gh_read_slha_qp => read_slha
      implicit none
      ...
      if(ierr.ne.0) then
         ...
         call p0_gg_gh_read_slha(27)
         call p0_gg_gh_read_slha_qp(27)
         close(27)
      end if
