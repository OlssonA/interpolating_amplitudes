module     p0_gg_gh_dipoles_qp
   use p0_gg_gh_config, only: ki => ki_qp
   use p0_gg_gh_color_qp, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2, &
       & T1T3, &
       & T2T3
   use p0_gg_gh_kinematics_qp, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_ff_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(0, ki), real(mH, ki))
...
end module p0_gg_gh_dipoles_qp
