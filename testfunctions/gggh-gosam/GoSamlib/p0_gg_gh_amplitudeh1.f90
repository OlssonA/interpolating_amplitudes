module    p0_gg_gh_amplitudeh1
   use p0_gg_gh_config, only: ki, &
       & reduction_interoperation
   use p0_gg_gh_color, only: numcs
   use p0_gg_gh_groups
   use ninjago_module, only: ki_nin
   use p0_gg_gh_ninjah1
...
!---#[ function finite_renormalisation:
   ...
   function     samplitude(scale2,ok,rational2,the_col0,opt_perm)
      ...
      use p0_gg_gh_globalsl1, only: col0,perm, use_perm, epspow
      use p0_gg_gh_globalsh1, &
     & only: init_lo, rat2
      use p0_gg_gh_abbrevd11h1, only: init_abbrevd11 => init_abbrev
      use p0_gg_gh_abbrevd5h1, only: init_abbrevd5 => init_abbrev
      use p0_gg_gh_abbrevd9h1, only: init_abbrevd9 => init_abbrev
      use p0_gg_gh_abbrevd1h1, only: init_abbrevd1 => init_abbrev
      use p0_gg_gh_abbrevd3h1, only: init_abbrevd3 => init_abbrev
      use p0_gg_gh_abbrevd7h1, only: init_abbrevd7 => init_abbrev
      use p0_gg_gh_groups
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use p0_gg_gh_ninjah1, only: ninja_reduce => ninja_reduce_group0
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   use p0_gg_gh_config, only: &
      & logfile, debug_nlo_diagrams
   use p0_gg_gh_globalsl1, only: epspow
   use p0_gg_gh_ninjah1, only: ninja_reduce => ninja_reduce_group1
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   use p0_gg_gh_config, only: &
      & logfile, debug_nlo_diagrams
   use p0_gg_gh_globalsl1, only: epspow
   use p0_gg_gh_ninjah1, only: ninja_reduce => ninja_reduce_group2
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
!---#] subroutine evaluate_group2:
end module p0_gg_gh_amplitudeh1
