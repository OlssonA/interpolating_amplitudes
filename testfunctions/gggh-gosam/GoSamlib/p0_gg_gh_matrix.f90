module     p0_gg_gh_matrix
   use p0_gg_gh_util, only: square
   use p0_gg_gh_util_qp, only: square_qp => square
   use p0_gg_gh_config, only: ki, ki_qp, &
     ...
     & PSP_chk_li1, PSP_chk_li2, PSP_chk_li3, PSP_chk_li4, PSP_chk_li5, PSP_chk_li6, &
     & reduction_interoperation_rescue, convert_to_cdr
   use p0_gg_gh_kinematics, only: &
       in_helicities, symmetry_factor, num_legs, &
       lo_qcd_couplings, corrections_are_qcd, num_light_quarks, num_gluons
   use p0_gg_gh_model, only: Nf, NC, sqrt2, init_functions
   use p0_gg_gh_model_qp, only: Nf_qp => Nf, NC_qp => NC, sqrt2_qp => sqrt2, &
     & init_functions_qp => init_functions
   use p0_gg_gh_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p0_gg_gh_color_qp, only: TR_qp => TR, CA_qp => CA, CF_qp => CF, &
     & incolors_qp => incolors, init_color_qp => init_color
   use p0_gg_gh_amplitudeh0, only: samplitudeh0l1 => samplitude, &
     &   finite_renormalisation0 => finite_renormalisation
   use p0_gg_gh_amplitudeh0_qp, only: samplitudeh0l1_qp => samplitude, &
     &   finite_renormalisation0_qp => finite_renormalisation
   use p0_gg_gh_amplitudeh1, only: samplitudeh1l1 => samplitude, &
     &   finite_renormalisation1 => finite_renormalisation
   use p0_gg_gh_amplitudeh1_qp, only: samplitudeh1l1_qp => samplitude, &
     &   finite_renormalisation1_qp => finite_renormalisation
   use p0_gg_gh_amplitudeh3, only: samplitudeh3l1 => samplitude, &
     &   finite_renormalisation3 => finite_renormalisation
   use p0_gg_gh_amplitudeh3_qp, only: samplitudeh3l1_qp => samplitude, &
     &   finite_renormalisation3_qp => finite_renormalisation
   use p0_gg_gh_amplitudeh4, only: samplitudeh4l1 => samplitude, &
     &   finite_renormalisation4 => finite_renormalisation
   use p0_gg_gh_amplitudeh4_qp, only: samplitudeh4l1_qp => samplitude, &
     &   finite_renormalisation4_qp => finite_renormalisation
   use p0_gg_gh_dipoles, only: insertion_operator, insertion_operator_qed
   use p0_gg_gh_dipoles_qp, only: insertion_operator_qp => insertion_operator, &
...
contains
   ...
   subroutine     banner()
      ...
      write(banner_ch,'(A74)') "|  \__/ \__/ (___/(_)(_)(_/\/\_)        Version 2.1.1 Rev: c319210       |"
      write(banner_ch,'(A74)') "|                                                                        |"
      write(banner_ch,'(A74)') "|                                 (c) The GoSam Collaboration 2011-2021  |"
      write(banner_ch,'(A74)') "|                                                                        |"
      ...
      write(banner_ch,'(A74)') "|            * Jannis Lang          <jannis.lang@kit.edu>                |"
      ...
      write(banner_ch,'(A74)') "|            * Ludovic Scyboz       <ludovic.scyboz@physics.ox.ac.uk>    |"
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      ! use p0_gg_gh_kinematics, only: adjust_kinematics
      use p0_gg_gh_kinematics_qp, only: adjust_kinematics_qp => adjust_kinematics
      use p0_gg_gh_model
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
      real(ki_qp), dimension(4, 4) :: vecs_qp
     ...
     &              mH/)
      ...
      ! RESCUE SYSTEM
      !write(101,*) 'reduction_interoperation', reduction_interoperation
      !write(101,*) 'PSP_check', PSP_check
      !write(101,*) 'icheck', icheck
      if(PSP_check) then
         ...
         !****************************************
         if(icheck.eq.2) then
            !write(101,*) '--double/quad rot test---'
            do irot = 1,4
            ...
            !if(icheck.eq.1) write(101,*) 'passed double vs double_rot', amp(2), amprot(2), fpprec1
            !if(icheck.eq.3) write(101,*) 'failed double vs double_rot', amp(2), amprot(2), fpprec1
         endif
         ...
         if(icheck.eq.3.and.PSP_rescue) then
            !write(101,*) 'double does not agree with double_rot, computing quad', amp(2), amprot(2)
            icheck = 1
            ...
            !if(icheck.eq.1) write(101,*) 'passed double/double_rot vs quad', amp(2), amprot(2), amp_qp(2), fpprec1, fpprec2
            !if(icheck.eq.3) write(101,*) 'failed double/double_rot vs quad', amp(2), amprot(2), amp_qp(2), fpprec1, fpprec2
         endif
         ...
         if(icheck.eq.3.and.PSP_rescue) then
            ...
            !if(icheck.eq.1) write(101,*) 'passed quad vs quad_rot', amp(2), amprot(2), amp_qp(2), amprot_qp(2), fpprec1
            !if(icheck.eq.3) write(101,*) 'failed quad vs quad_rot', amp(2), amprot(2), amp_qp(2), amprot_qp(2), fpprec1
         endif
         ...
         if(icheck.eq.3.and.PSP_rescue) then
            ...
            scale2_qp = real(scale2,ki_qp)
            vecs_qp = vecs
            ! call refine_momenta_to_qp(4,vecs,vecs_qp,1+1,scales2)
            call adjust_kinematics_qp(vecs_qp)
            call samplitudel01_qp(vecs_qp, scale2_qp, amp_qp, rat2_qp, ok, h)
            call ir_subtraction_qp(vecs_qp, scale2_qp, irp_qp, h)
            ampres = real(amp_qp,ki)
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p0_gg_gh_config, only: logfile
      use p0_gg_gh_kinematics, only: init_event
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
   ...
   subroutine     samplitudel01_qp(vecs, scale2, amp, rat2, ok, h)
      use p0_gg_gh_config, only: &
         ...
         & renormalisation, renorm_beta, renorm_mqwf, renorm_decoupling, &
         & renorm_logs, renorm_mqse, renorm_yukawa, nlo_prefactors
      use p0_gg_gh_kinematics_qp, only: &
         & inspect_kinematics, init_event
      use p0_gg_gh_model_qp
      use p0_gg_gh_dipoles_qp, only: pi
      implicit none
      real(ki_qp), dimension(4, 4), intent(in) :: vecs
   ...
   function     samplitudel0_qp(vecs, h) result(amp)
      use p0_gg_gh_config, only: logfile
      use p0_gg_gh_kinematics_qp, only: init_event
      implicit none
      real(ki_qp), dimension(4, 4), intent(in) :: vecs
      integer, optional, intent(in) :: h
      ...
      logical, dimension(0:7) :: eval_heli
      real(ki_qp), dimension(4, 4) :: pvecs
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      ...
      res(2,1) = res(1,2)
      res(1,3) = square(color_vector,T1T3)
      res(3,1) = res(1,3)
      res(2,2) = square(color_vector,T2T2)
      res(2,2) = res(2,2)
      res(2,3) = square(color_vector,T2T3)
      res(3,2) = res(2,3)
      res(3,3) = square(color_vector,T3T3)
      res(3,3) = res(3,3)
   ...
   pure subroutine OLP_color_correlated_lo(color_vector,res)
      ...
      res(2) = square(color_vector,T1T3)    
      res(3) = square(color_vector,T2T3)   
   ...
   subroutine OLP_spin_correlated_lo2(vecs, ampsc)
      ...
      mp  = 0.0_ki &
      &          + square_1_3_sc(heli_amp0,heli_amp1) &
      &          + square_1_3_sc(heli_amp2,heli_amp3) &
      &          + square_1_3_sc(heli_amp4,heli_amp5) &
      &          + square_1_3_sc(heli_amp6,heli_amp7)

      ampsc(2*(1-1)+2*(3-1)*num_legs+1)   = ampsc(2*(1-1)+2*(3-1)*num_legs +1) + real(mp, ki)
      ampsc(2*(1-1)+2*(3-1)*num_legs+2) = ampsc(2*(1-1)+2*(3-1)*num_legs + 2)  + real(aimag(mp),ki)
      ...
      mp  = 0.0_ki &
      &          + square_2_3_sc(heli_amp0,heli_amp2) &
      &          + square_2_3_sc(heli_amp1,heli_amp3) &
      &          + square_2_3_sc(heli_amp4,heli_amp6) &
      &          + square_2_3_sc(heli_amp5,heli_amp7)

      ampsc(2*(2-1)+2*(3-1)*num_legs+1)   = ampsc(2*(2-1)+2*(3-1)*num_legs +1) + real(mp, ki)
      ampsc(2*(2-1)+2*(3-1)*num_legs+2) = ampsc(2*(2-1)+2*(3-1)*num_legs + 2)  + real(aimag(mp),ki)
      ...
      mp  = 0.0_ki &
      &          + square_3_1_sc(heli_amp0,heli_amp4) &
      &          + square_3_1_sc(heli_amp1,heli_amp5) &
      &          + square_3_1_sc(heli_amp2,heli_amp6) &
      &          + square_3_1_sc(heli_amp3,heli_amp7)

      ampsc(2*(3-1)+2*(1-1)*num_legs+1)   = ampsc(2*(3-1)+2*(1-1)*num_legs +1) + real(mp, ki)
      ampsc(2*(3-1)+2*(1-1)*num_legs+2) = ampsc(2*(3-1)+2*(1-1)*num_legs + 2)  + real(aimag(mp),ki)
      ...
      mp  = 0.0_ki &
      &          + square_3_2_sc(heli_amp0,heli_amp4) &
      &          + square_3_2_sc(heli_amp1,heli_amp5) &
      &          + square_3_2_sc(heli_amp2,heli_amp6) &
      &          + square_3_2_sc(heli_amp3,heli_amp7)

      ampsc(2*(3-1)+2*(2-1)*num_legs+1)   = ampsc(2*(3-1)+2*(2-1)*num_legs +1) + real(mp, ki)
      ampsc(2*(3-1)+2*(2-1)*num_legs+2) = ampsc(2*(3-1)+2*(2-1)*num_legs + 2)  + real(aimag(mp),ki)
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_gh_color, only: cmat => CC
      implicit none
   ...
   pure function square_1_2_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_gh_color, only: cmat => T1T2
   ...
   pure function square_1_3_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_gh_color, only: cmat => T1T3
   ...
   pure function square_2_3_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_gh_color, only: cmat => T2T3
      ...
      v2 = conjg(color_vector1)
      amp = sum(v1(:) * v2(:))
   end function  square_2_3_sc
      
   pure function square_3_1_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_gh_color, only: cmat => T1T3
      ...
      v2 = conjg(color_vector1)
      amp = sum(v1(:) * v2(:))
   end function  square_3_1_sc
      
   pure function square_3_2_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_gh_color, only: cmat => T2T3
