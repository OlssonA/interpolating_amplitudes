...
subroutine     ninja_reduce_group0(scale2,tot,totr,ok)
   ...
   use p0_gg_gh_d11h1l1_qp, only: numerator_diagram11 => numerator_ninja
   use p0_gg_gh_d11h1l121_qp, only: numerator_tmu_diagram11 => numerator_tmu
   use p0_gg_gh_d11h1l131_qp, only: numerator_t3_diagram11 => numerator_t3
   use p0_gg_gh_d11h1l132_qp, only: numerator_t2_diagram11 => numerator_t2
   ...
   s_mat(2,1) = s_mat(1,2)
   s_mat(1,3) = real(-es23-es12+mH**2-2.0_ki*mT**2, ki_nin)
   s_mat(3,1) = s_mat(1,3)
...
subroutine     ninja_reduce_group1(scale2,tot,totr,ok)
   ...
   use p0_gg_gh_kinematics_qp
   use p0_gg_gh_model_qp
   use p0_gg_gh_d5h1l1_qp, only: numerator_diagram5 => numerator_ninja
   use p0_gg_gh_d5h1l121_qp, only: numerator_tmu_diagram5 => numerator_tmu
   use p0_gg_gh_d5h1l131_qp, only: numerator_t3_diagram5 => numerator_t3
   use p0_gg_gh_d5h1l132_qp, only: numerator_t2_diagram5 => numerator_t2
   use p0_gg_gh_d9h1l1_qp, only: numerator_diagram9 => numerator_ninja
   use p0_gg_gh_d9h1l121_qp, only: numerator_tmu_diagram9 => numerator_tmu
   use p0_gg_gh_d9h1l131_qp, only: numerator_t3_diagram9 => numerator_t3
   use p0_gg_gh_d9h1l132_qp, only: numerator_t2_diagram9 => numerator_t2
   use p0_gg_gh_globalsl1_qp, only: epspow
   ...
   !-----------#] initialize invariants
      ...
      !------#[ sum over reduction of single diagrams:
         ...
         call quadninja_diagram(numerator_diagram9, numerator_t3_diagram9, nume&
         &rator_t2_diagram9, &
...
subroutine     ninja_reduce_group2(scale2,tot,totr,ok)
   ...
   use p0_gg_gh_kinematics_qp
   use p0_gg_gh_model_qp
   use p0_gg_gh_d1h1l1_qp, only: numerator_diagram1 => numerator_ninja
   use p0_gg_gh_d1h1l121_qp, only: numerator_tmu_diagram1 => numerator_tmu
   use p0_gg_gh_d1h1l131_qp, only: numerator_t3_diagram1 => numerator_t3
   use p0_gg_gh_d1h1l132_qp, only: numerator_t2_diagram1 => numerator_t2
   use p0_gg_gh_d3h1l1_qp, only: numerator_diagram3 => numerator_ninja
   use p0_gg_gh_d3h1l121_qp, only: numerator_tmu_diagram3 => numerator_tmu
   use p0_gg_gh_d3h1l131_qp, only: numerator_t3_diagram3 => numerator_t3
   use p0_gg_gh_d3h1l132_qp, only: numerator_t2_diagram3 => numerator_t2
   use p0_gg_gh_d7h1l1_qp, only: numerator_diagram7 => numerator_ninja
   use p0_gg_gh_d7h1l121_qp, only: numerator_tmu_diagram7 => numerator_tmu
   use p0_gg_gh_d7h1l131_qp, only: numerator_t3_diagram7 => numerator_t3
   use p0_gg_gh_d7h1l132_qp, only: numerator_t2_diagram7 => numerator_t2
   use p0_gg_gh_globalsl1_qp, only: epspow
   ...
   s_mat(3,2) = s_mat(2,3)
   s_mat(2,4) = real(mH**2-es23-es12-2.0_ki*mT**2, ki_nin)
   s_mat(4,2) = s_mat(2,4)
   s_mat(3,3) = real(-2.0_ki*mT**2, ki_nin)
   s_mat(3,4) = real(-2.0_ki*mT**2, ki_nin)
   s_mat(4,3) = s_mat(3,4)
...
!-----#] subroutine ninja_reduce_group2:
!---#] reduce groups with ninja:
end module p0_gg_gh_ninjah1_qp
